...
def get_challs(args):
    ...
    if response.status_code != 200:
        print('❌', f'Failed to get game info from {args.url}, status code: {response.status_code}')
        sys.exit(1)
...
def get_one_chall(args, id: int, headers: dict, game_title: str):
    ...
    url = f'{args.url}/challenges/{id}'
    response = requests.get(url, headers=headers)
    if response.status_code != 200:
        print('❌', f'Failed to get challenge info from {url}, status code: {response.status_code}')
        return
    ...
    size = response_json['context']['fileSize']      # may be null
    
    if remote_path is None:
        print('⏩', f'{tag}/{name}'.ljust(24), 'has no attachment')
        return
    
    if size is not None and size > args.max_size:
        print('🤯', f'{tag}/{name}'.ljust(24), f'is too large ({format(size, ",")} bytes)')
        return
    ...
    size = raw_size if size is None else max(size, raw_size)
    ...
    for chunk in response.iter_content(chunk_size=65536):
        if chunk:
            ...
            print('\r📥',
                  f'{tag}/{name}'.ljust(24),
                  '>' * min(got_size*50//size, 50) + '_' * min(50-got_size*50//size, 50),
...
def arg_parse():
    ...
    parser.add_argument('-d', '--root-directory', type=str, default='{game}', help='root directory, default is `pwd`/{game}, e.g. ./LRCTF 2024')
    parser.add_argument('-f', '--file-path', type=str, default='{tag}/{chall}/{chall}_{raw}', help='style of file path, default is {tag}/{chall}/{chall}_{raw}, e.g. misc/sign in/sign in_attachment_deadbeef.zip')
    ...
    parser.add_argument('-s', '--max-size', type=float, default=10.0, help='max file size in MB, larger than this will be skipped, default is 10.0, set to 0 to disable')
    parser.add_argument('-o', '--overwrite', action="store_true", help='if specified, existing files will be replaced instead of skipped')
    
    direction_group = parser.add_argument_group('direction options, default is ALL, you can specify like -mwp')
    direction_group.add_argument('-E', '--except-mode', action="store_true", help='-w means ONLY download web, while -E -w means download everything else EXCEPT web')
    direction_group.add_argument('-m', '--misc', action='store_true')
    direction_group.add_argument('-c', '--crypto', action='store_true')
    direction_group.add_argument('-p', '--pwn', action='store_true')
    direction_group.add_argument('-w', '--web', action='store_true')
    direction_group.add_argument('-r', '--reverse', action='store_true')
    direction_group.add_argument('--blockchain', action='store_true')
    direction_group.add_argument('--forensics', action='store_true')
    direction_group.add_argument('--hardware', action='store_true')
    direction_group.add_argument('--mobile', action='store_true')
    direction_group.add_argument('--ppc', action='store_true')
    direction_group.add_argument('--ai', action='store_true')
    ...
    if args.token is None:
        args.token = input('\nEnter GZCTF_TOKEN cookie value here: ').strip()
    args.token = args.token.replace('GZCTF_TOKEN=', '').strip()

    args.max_size = args.max_size * 1024 * 1024 if args.max_size > 0 else float('inf')

    direction_list = ['misc', 'crypto', 'pwn', 'web', 'reverse', 'blockchain', 'forensics', 'hardware', 'mobile', 'ppc', 'ai']
    allowlist = direction_list.copy()
    if any(getattr(args, direction) for direction in direction_list):
        for direction in direction_list:
            if bool(getattr(args, direction)) ^ (not args.except_mode):
                allowlist.remove(direction)
    
    args.allowlist = allowlist
    for direction in direction_list:
        delattr(args, direction)
